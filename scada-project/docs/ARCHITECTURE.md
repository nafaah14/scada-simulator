...
- **All six screens are ported** to canvas JSON and driven by tags:
  `Common.Overview`, `Common.Fuel`, `Common.StartAir`, `G1.Temp`, `G1.Fuel`
  and `G1.Control` — 449 bound elements over ~396 runtime tags, every one
  resolving. The legacy standalone HTML under `mockups/screens/` is kept only
  as visual reference; nothing loads it any more.
- Screens are generated by `tools/screens/*.mjs` through a small builder
  (`tools/screen-builder.mjs`) so geometry stays readable. Re-running
  `npm run generate` overwrites, so export from the editor first if a screen
  has been tuned there.
- All six gensets are simulated, with G4 held out of service so the plant
  reads as a realistic mixed state (and the standing Genset_4 alarm has a
  cause).

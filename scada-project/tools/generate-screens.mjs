/* =====================================================================
   ...
   exists to lay down the initial geometry, not to be re-run over edits.
   Re-running overwrites, so export from the editor first if unsure.
   ===================================================================== */
...
/* ---------------------------------------------------------------------
   G1.Temp — engine cylinder + alternator temperatures
...
/* ------------------------------------------------------------------ */
const screens = [g1Temp()];

const index = screens.map(s => ({
  screen_id: s.screen_id,
  title: s.title,
  unit: s.unit || null,
  file: s.screen_id.toLowerCase().replace(/[^a-z0-9]+/g, '-') + '.json'
}));

screens.forEach((s, i) => {
  const path = join(OUT, index[i].file);
  writeFileSync(path, JSON.stringify(s, null, 2) + '\n');
  console.log('wrote', index[i].file, '—', s.elements.length, 'elements');
});
writeFileSync(join(OUT, 'index.json'), JSON.stringify(index, null, 2) + '\n');
console.log('wrote index.json —', index.length, 'screen(s)');
